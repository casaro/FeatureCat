function [] = FeatureCat_createWeightingTern(varargin)
    ...
    resultFilename = 'results_objectiveWeighting.mat';
    ...
    for a=0:0.1:1
        for b=0.0:0.1:(1-a)
            ...
            Weights = [Weights; a b c length(dictS)];
        end
    end
	...
	set(gcf, 'PaperPosition', [-2 0 12 5]);
	set(gcf, 'PaperSize',[12, 5]);
    ternpcolor(Results(:,2),Results(:,3),max(Results(:,6), 0.87));
    shading interp;
    vertexlabel('max', 'min', 'discrete', 0.08);
    colorbar('eastoutside');
    fName = '/mounts/Users/student/sascha/paper/FeatureCat/objective_weighting2.pdf';
    saveas(h,fName);
    close(h);
end
